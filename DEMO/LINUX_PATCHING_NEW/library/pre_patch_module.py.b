...
def convert_txt_to_csv(backup_path, ip_address,server):
    """
    ...
    """
    folder_path = os.path.join(backup_path, f"{ip_address}_Prepatch_*")
    output_csv_file = os.path.join(backup_path, f"{ip_address}_prepatch_output.csv")
...
def main():
    module_args = dict(
        ...
        server_config_list=dict(type='str', required=True)
    )
    ...
    server_config_list = module.params['server_config_list'] 
    ...
    if "network_status" in server_config_list:
        network_status_results = backup_network_status(backup_path, ip_address, "Prepatch")
    else:
        network_status_results = {}

    if "pswd_groups_sudoers" in server_config_list:
        pswd_groups_sudoers_results = backup_pswd_groups_sudoers(backup_path, ip_address, "Prepatch")
    else:
        pswd_groups_sudoers_results = {}

    if "routing_info" in server_config_list:
        backup_routing_dg_info_results = backup_routing_info(backup_path, ip_address, "Prepatch")
    else:
        backup_routing_dg_info_results = {}

    if "memory_swap_info" in server_config_list:
        backup_memory_swap_info_results = backup_memory_info(backup_path, ip_address, "Prepatch")
    else:
        backup_memory_swap_info_results = {}

    if "server_info" in server_config_list:
        backup_server_info_results = backup_server_status(backup_path, ip_address, "Prepatch")
    else:
        backup_server_info_results = {}

    if "system_build_date" in server_config_list:
        backup_system_build_date_results = backup_system_build_date(backup_path, ip_address, "Prepatch")
    else:
        backup_system_build_date_results = {}

    if "selinux_info" in server_config_list:
        backup_selinux_info_results = backup_selinux_status(backup_path, ip_address)
    else:
        backup_selinux_info_results = {}

    if "network_interface_pre" in server_config_list:
        backup_network_interface_pre_results = backup_network_info(backup_path, ip_address)
    else:
        backup_network_interface_pre_results = {}

    if "network_config" in server_config_list:
        backup_network_config_results = backup_network_config(backup_path, ip_address)
    else:
        backup_network_config_results = {}

    if "network_link_status" in server_config_list:
        backup_network_link_status_results = backup_network_link_status(backup_path, ip_address)
    else:
        backup_network_link_status_results = {}

    if "kernel_status" in server_config_list:
        backup_kernel_status_results = backup_kernel_info(backup_path, ip_address, "Prepatch")
    else:
        backup_kernel_status_results = {}

    if "host_config" in server_config_list:
        backup_host_config_results = backup_hosts_info(backup_path, ip_address, "Prepatch")
    else:
        backup_host_config_results = {}

    if "dns_resolvers_file" in server_config_list:
        backup_dns_resolvers_file_results = backup_dns_resolvers_info(backup_path, ip_address, "Prepatch")
    else:
        backup_dns_resolvers_file_results = {}

    if "db_avail_and_running_status" in server_config_list:
        backup_db_avail_and_running_status_results = backup_db_avail_and_running_status(backup_path, ip_address, "Prepatch")
    else:
        backup_db_avail_and_running_status_results = {}

    if "satellite_subscription_status" in server_config_list:
        backup_satellite_subscription_status_file = backup_satellite_subscription_status(backup_path, ip_address, "Prepatch")
    else:
        backup_satellite_subscription_status_file = {}

    if "redhat_release_file" in server_config_list:
        backup_redhat_release_file_results = backup_redhat_release_info(backup_path, ip_address, "Prepatch")
    else:
        backup_redhat_release_file_results = {}

    if "firewall_status_and_config" in server_config_list:
        backup_firewall_status_and_config_file_results = backup_firewall_status_and_config(backup_path, ip_address, "Prepatch")
    else:
        backup_firewall_status_and_config_file_results = {}

    if "info_host_information" in server_config_list:
        backup_info_host_information_file_results = backup_info_host_information(backup_path, ip_address, "Prepatch")
    else:
        backup_info_host_information_file_results = {}

    if "info_uptime" in server_config_list:
        backup_info_uptime_results = backup_info_uptime(backup_path, ip_address, "Prepatch")
    else:
        backup_info_uptime_results = {}

    if "info_cpu_information" in server_config_list:
        backup_info_cpu_information_results = backup_info_cpu_information(backup_path, ip_address, "Prepatch")
    else:
        backup_info_cpu_information_results = {}

    if "info_blkid" in server_config_list:
        backup_info_blkid_results = backup_info_blkid(backup_path, ip_address, "Prepatch")
    else:
        backup_info_blkid_results = {}

    if "info_last_reboot" in server_config_list:
        backup_info_last_reboot_results = backup_info_last_reboot(backup_path, ip_address, "Prepatch")
    else:
        backup_info_last_reboot_results = {}

    if "info_kernel_version" in server_config_list:
        backup_info_kernel_version_results = backup_info_kernel_version(backup_path, ip_address, "Prepatch")
    else:
        backup_info_kernel_version_results = {}

    if "info_disk_available" in server_config_list:
        backup_info_disk_available_results = backup_info_disk_available(backup_path, ip_address, "Prepatch")
    else:
        backup_info_disk_available_results = {}

    if "info_current_FS_utilization" in server_config_list:
        backup_info_current_FS_utilization_results = backup_info_current_FS_utilization(backup_path, ip_address, "Prepatch")
    else:
        backup_info_current_FS_utilization_results = {}
    ...
    csv_file_path = convert_txt_to_csv(backup_path, ip_address,server_config_list)
