...
def backup_dns_resolvers_info(backup_path, ip_address, stage):
    """
    Backs up the /etc/resolv.conf configuration and date/time before or after the patch.
    ...
    try:
        ...
        backup_file = os.path.join(backup_dir, f'dns_resolvers_info_{stage}.txt')
 
        final_dic = {}
 
        # DNS resolvers information
        resolvers_info_result = subprocess.run(['sudo', 'cat', '/etc/resolv.conf'], stdout=subprocess.PIPE)
        resolvers_info = resolvers_info_result.stdout.decode('utf-8').strip()
        final_dic['dns_resolvers_info'] = resolvers_info
 
        # Write the collected information to the backup file
        with open(backup_file, 'w') as f:
            json.dump(final_dic, f)
 
        return {"changed": True, "msg": f"Backup DNS resolvers info saved to {backup_file}"}
 
    except Exception as e:
        return {"failed": True, "msg": f"Backup failed: {str(e)}"}
 
def compare_dns_resolvers_info(backup_path, ip_address):
    """
    Compares the pre-patch and post-patch DNS resolvers information and saves the differences to a file.
    ...
    try:
        ...
        pre_patch_file = os.path.join(pre_patch_dir, 'dns_resolvers_info_Prepatch.txt')
        post_patch_file = os.path.join(post_patch_dir, 'dns_resolvers_info_Postpatch.txt')
        ...
        for key in pre_patch_info:
            if key in post_patch_info and pre_patch_info[key] != post_patch_info[key]:
                ...
                csv_res = csv_convert_function(ip_address,backup_path,"info_dns_resolvers",pre_patch_info[key],post_patch_info[key])
        ...
        if diff_info != {}:
            ...
            diff_info_dateTime_file = os.path.join(diff_backup_dir, 'diff_dns_resolvers_info.txt')
           
            with open(diff_info_dateTime_file, 'w') as f:
                json.dump(diff_info, f, indent=4)
           
            return {"changed": True, "msg": f"DNS resolvers info differ:\n{diff_info}"}
        else:
            return {"changed": False, "msg": "DNS resolvers info are identical"}
