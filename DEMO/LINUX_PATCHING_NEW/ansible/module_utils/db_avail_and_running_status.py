...
def backup_db_avail_and_running_status(backup_path, ip_address, stage):
    """
    ...
    try:
        ...
        backup_file = os.path.join(backup_dir, f'db_avail_and_running_status_{stage}.txt')
 
        final_dic = {}
 
        # db_avail_and_running_status
        Database_availability_result = subprocess.run(["ps", "-ef","|", "egrep", "-i", "'pmon|postgres'", "|", "grep", "-v", "grep"], stdout=subprocess.PIPE, shell=True)
        Database_availability_result_info = Database_availability_result.stdout.decode('utf-8').strip()
        final_dic['Database availability'] = Database_availability_result_info

        
        process_result = subprocess.run(['ps','aux'], stdout=subprocess.PIPE)
        process_result = process_result.stdout.decode('utf-8').strip()
        final_dic['ps aux'] = process_result
 
        # Write the collected information to the backup file
        with open(backup_file, 'w') as f:
            json.dump(final_dic, f)
 
        return {"changed": True, "msg": f"Backup db_avail_and_running_status info saved to {backup_file}"}
 
    except Exception as e:
        return {"failed": True, "msg": f"Backup failed: {str(e)}"}
 
def compare_db_avail_and_running_status(backup_path, ip_address):
    """
    ...
    try:
        ...
        pre_patch_file = os.path.join(pre_patch_dir, 'db_avail_and_running_status_Prepatch.txt')
        post_patch_file = os.path.join(post_patch_dir, 'db_avail_and_running_status_Postpatch.txt')
        ...
        for key in pre_patch_info:
            if key in post_patch_info and pre_patch_info[key] != post_patch_info[key]:
                ...
                csv_res = csv_convert_function(ip_address,backup_path,"info_db_avail_and_running_status",pre_patch_info[key],post_patch_info[key])
        ...
        if diff_info != {}:
            ...
            diff_info_dateTime_file = os.path.join(diff_backup_dir, 'diff_db_avail_and_running_status.txt')
           
            with open(diff_info_dateTime_file, 'w') as f:
                json.dump(diff_info, f, indent=4)
           
            return {"changed": True, "msg": f"Database_availability_result differ"}
        else:
            return {"changed": False, "msg": "Database_availability_result are identical"}
