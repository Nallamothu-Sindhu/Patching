...
def backup_info_kernel_version(backup_path, ip_address, stage):

    """

    Backs up kernel version before or after the patch.
    ...
    try:
        ...
        backup_file = os.path.join(backup_dir, f'info_and_kernel_version_{stage}.txt')

        final_dic = {}
        
        res = subprocess.run(['uname', '-r'], stdout=subprocess.PIPE)

        kernel_version = res.stdout.decode('utf-8').strip()

        final_dic['kernel_version'] = kernel_version

        with open(backup_file, 'w') as f:

            json.dump(final_dic, f)

        return {"changed": True, "msg": f"Backup info and kernel version saved to {backup_file}"}

    except Exception as e:

        return {"failed": True, "msg": f"Backup failed: {e}"}
 
def compare_info_kernel_version(backup_path, ip_address):
    ...
    try:
        ...
        pre_patch_file = os.path.join(pre_patch_dir, 'info_and_kernel_version_Prepatch.txt')

        post_patch_file = os.path.join(post_patch_dir, 'info_and_kernel_version_Postpatch.txt')
        ...
        for key in pre_patch_info:
            if key in post_patch_info and pre_patch_info[key] != post_patch_info[key]:
               ...
               csv_res = csv_convert_function(ip_address,backup_path,"info_kernel_version",pre_patch_info[key],post_patch_info[key])
        ...
        if diff_info != {}:
            ...
            diff_info_dateTime_file = os.path.join(diff_backup_dir, 'diff_info_kernel_version.txt')
            with open(diff_info_dateTime_file, 'w') as f:
                 json.dump(diff_info, f, indent=4)
            return {"changed": True, "msg": f"kernel version  differ:\n{diff_info}"}
