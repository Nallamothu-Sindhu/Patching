...
def backup_firewall_status_and_config(backup_path, ip_address, stage):
    ...
    try:
        ...
        backup_file = os.path.join(backup_dir, f'firewall_status_and_config_{stage}.txt')

        final_dic = {}

        # Networker status
        
        systemctl_status_firewalld_result = subprocess.run(['systemctl','status','firewalld'], stdout=subprocess.PIPE)
        systemctl_status_firewalld_result = systemctl_status_firewalld_result.stdout.decode('utf-8').strip()
        final_dic['systemctl_status_firewalld_result'] = systemctl_status_firewalld_result

        
        service_iptables_status_result = subprocess.run(['service','iptables','status'], stdout=subprocess.PIPE)
        service_iptables_status_result = service_iptables_status_result.stdout.decode('utf-8').strip()
        final_dic['service_iptables_status_result'] = service_iptables_status_result

        firewallcmd_result = subprocess.run(['firewall-cmd', '--list-all'], stdout=subprocess.PIPE)
        firewallcmd_result = firewallcmd_result.stdout.decode('utf-8').strip()
        final_dic['firewallcmd_result'] = firewallcmd_result

        iptables_result = subprocess.run(['iptables', '--L'], stdout=subprocess.PIPE)
        iptables_result = iptables_result.stdout.decode('utf-8').strip()
        final_dic['iptables_result'] = iptables_result
        ...
        return {"changed": True, "msg": f"Backup firewall_status_and_config_ saved to {backup_file}"}

    except Exception as e:

        return {"failed": True, "msg": f"Backup failed: {e}"}
 
def compare_firewall_status_and_config(backup_path, ip_address):
    ...
    try:
        ...
        pre_patch_file = os.path.join(pre_patch_dir, 'firewall_status_and_config_Prepatch.txt')

        post_patch_file = os.path.join(post_patch_dir, 'firewall_status_and_config_Postpatch.txt')
        ...
        for key in pre_patch_info:
            if key in post_patch_info and pre_patch_info[key] != post_patch_info[key]:  
               ...
               csv_res = csv_convert_function(ip_address,backup_path,"info_firewall_status",pre_patch_info[key],post_patch_info[key])
        ...
        if diff_info != {}:
            ...
            diff_info_dateTime_file = os.path.join(diff_backup_dir, 'diff_firewall_status_and_config.txt')
            with open(diff_info_dateTime_file, 'w') as f:
                json.dump(diff_info, f, indent=4)
            return {"changed": True, "msg": f"firewall_status_and_config differ:\n{diff_info}"}
        else:
            
            return {"changed": True, "msg": f"firewall_status_and_config is identical"}
    except Exception as e:
