...
def backup_network_config(backup_path, ip_address,stage):
    try:
        ...
        backup_file = os.path.join(pre_patch_dir, 'network_config_{stage}.txt')
        
        final_dic = {}
        res = subprocess.run(['ip', 'a'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        
        if res.returncode != 0:
            return {"failed": True, "msg": f"ip command failed: {res.stderr.decode('utf-8').strip()}"}
        
        final_dic['network_config'] = res.stdout.decode('utf-8').strip()
        
        with open(backup_file, 'w') as f:
            json.dump(final_dic, f, indent=4)
        
        return {"changed": True, "msg": f"Backup network interface configuration saved to {backup_file}"}
    
    except Exception as e:
        return {"failed": True, "msg": f"Backup network interface configuration failed: {e}"}

def compare_network_config(backup_path, ip_address):
    try:
        timestamp = datetime.now().strftime("%Y-%m-%d")
        ...
        post_backup_file = os.path.join(post_patch_dir, 'network_config_Postpatch.txt')
        ...
        if not pre_patch_dirs:
            return {"failed": True, "msg": "No pre-patch network configuration file found to compare"}
        
        pre_patch_dir = os.path.join(backup_path, max(pre_patch_dirs))
        pre_patch_file = os.path.join(pre_patch_dir, 'network_config_Prepatch.txt')
        
        if not os.path.exists(pre_patch_file):
            return {"failed": True, "msg": "Pre-patch network configuration file not found in the most recent pre-patch directory"}
        ...
        for key in pre_patch_info:
            if key in post_patch_info and pre_patch_info[key] != post_patch_info[key]:
                ...
                csv_res = csv_convert_function(ip_address,backup_path,"info_network_config",pre_patch_info[key],post_patch_info[key])
        ...
        if diff_info != {}:
            ...
            diff_info_file = os.path.join(diff_backup_dir, 'diff_network_config.txt')
            with open(diff_info_file, 'w') as f:
                 json.dump(diff_info, f, indent=4)
        
        
            return {"changed": True, "msg": f"Network interface configuration differences saved to {diff_info_file}"}
        else:
            return {"changed": False, "msg": "Network interface configuration is identical between pre-patch and post-patch"}

        
    
    except Exception as e:
        return {"failed": True, "msg": f"Comparison of network interface configuration failed: {e}"}
