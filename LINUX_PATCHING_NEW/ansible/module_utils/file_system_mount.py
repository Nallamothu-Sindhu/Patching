...
def backup_mount_list(backup_path,ip_address,stage):
    """
    Backs up the file system status mount before or after the patch.
    ...
    try:
        ...
        backup_file = os.path.join(backup_dir, f'file_system_mount_{stage}.txt')
        final_dic = {}
 
        
        

        mount_result= subprocess.run(['mount','-l'],stdout=subprocess.PIPE)
        mount_status= mount_result.stdout.decode('utf-8').strip()
        final_dic['mount_status']= mount_status
        with open(backup_file, 'w') as f:
            json.dump(final_dic, f)
 
        return {"changed": True, "msg": f"Backup file system mount status saved to {backup_file}"}
 
    except Exception as e:
        return {"failed": True, "msg": f"Backup failed: {str(e)}"}
   
def compare_mount_list(backup_path, ip_address):
    """
    Compares the pre-patch and post-patch file system mount information and saves the differences to a file.
    ...
    try:
        ...
        pre_patch_file = os.path.join(pre_patch_dir, 'file_system_mount_Prepatch.txt')
        post_patch_file = os.path.join(post_patch_dir, 'file_system_mount_Postpatch.txt')
        ...
        for key in pre_patch_info:
            if key in post_patch_info and pre_patch_info[key] != post_patch_info[key]:
                ...
                csv_res = csv_convert_function(ip_address,backup_path,"info_mount",pre_patch_info[key],post_patch_info[key])
        ...
        if diff_info != {}:
            ...
            diff_info_dateTime_file = os.path.join(diff_backup_dir, 'diff_file_system_mount__info.txt')
 
            with open(diff_info_dateTime_file, 'w') as f:
                json.dump(diff_info, f, indent=4)
 
            return {"changed": True, "msg": f" file system mount status differences {diff_info}"}
        else:
            return {"changed": False, "msg": "File system mount status info is identical"}
