...
def backup_info_disk_available(backup_path, ip_address, stage):

    """

    Backs up disk available on the system before or after the patch.
    ...
    try:
        ...
        backup_file = os.path.join(backup_dir, f'info_and_disk_available_{stage}.txt')
        ...
        return {"changed": True, "msg": f"Backup info and Disks available on the system saved to {backup_file}"}

    except Exception as e:

        return {"failed": True, "msg": f"Backup failed: {e}"}
 
def compare_info_disk_available(backup_path, ip_address):
    ...
    try:
        ...
        pre_patch_file = os.path.join(pre_patch_dir, 'info_and_disk_available_Prepatch.txt')

        post_patch_file = os.path.join(post_patch_dir, 'info_and_disk_available_Postpatch.txt')
        ...
        for key in pre_patch_info:
            if key in post_patch_info and pre_patch_info[key] != post_patch_info[key]:
               ...
               csv_res = csv_convert_function(ip_address,backup_path,"info_disk_available",pre_patch_info[key],post_patch_info[key])
        ...
        if diff_info != {}:
           ...
           diff_info_dateTime_file = os.path.join(diff_backup_dir, 'diff_info_disk_available.txt')
           with open(diff_info_dateTime_file, 'w') as f:
                json.dump(diff_info, f, indent=4)
           return {"changed": True, "msg": f"disk information differ:\n{diff_info}"}
