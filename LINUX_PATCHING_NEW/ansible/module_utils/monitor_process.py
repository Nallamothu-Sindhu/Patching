...
def backup_monitor_process(backup_path, ip_address, stage):
    ...
    try:
        ...
        backup_file = os.path.join(backup_dir, f'monitor_process_{stage}.txt')
        ...
        final_dic['monitor processes'] = process_result
        ...
        return {"changed": True, "msg": f"Backup info and date/time saved to {backup_file}"}

    except Exception as e:

        return {"failed": True, "msg": f"Backup failed: {e}"}
 
def compare_monitor_process(backup_path, ip_address):
    ...
    try:
        ...
        pre_patch_file = os.path.join(pre_patch_dir, 'monitor_process_Prepatch.txt')

        post_patch_file = os.path.join(post_patch_dir, 'monitor_process_Postpatch.txt')
        ...
        for key in pre_patch_info:
            if key in post_patch_info and pre_patch_info[key] != post_patch_info[key]:
               ...
               csv_res = csv_convert_function(ip_address,backup_path,"info_monitor_process",pre_patch_info[key],post_patch_info[key])
        ...
        if diff_info != {}: 
            ...
            diff_info_dateTime_file = os.path.join(diff_backup_dir, 'diff_monitor_process.txt')
            with open(diff_info_dateTime_file, 'w') as f:
                json.dump(diff_info, f, indent=4)
            return {"changed": True, "msg": f"date and time information differ:\n{diff_info}"}
        else:
            return {"changed": True, "msg": f"date and time information are identical"}
    except Exception as e:
