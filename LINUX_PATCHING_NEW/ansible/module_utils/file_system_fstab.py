...
def backup_fstab_list(backup_path, ip_address, stage):
    """
    Backs up the /etc/fstab configuration and date/time before or after the patch.
    ...
    try:
        ...
        backup_file = os.path.join(backup_dir, f'fstab_{stage}.txt')
 
        final_dic = {}
 
        fstab_result = subprocess.run(['sudo', 'cat', '/etc/fstab'], stdout=subprocess.PIPE)
        fstab_status = fstab_result.stdout.decode('utf-8').strip()
 
        final_dic['subscription_status'] = fstab_status
        ...
        return {"changed": True, "msg": f"Backup fstab info saved to {backup_file}"}
 
    except Exception as e:
        return {"failed": True, "msg": f"Backup failed: {str(e)}"}
 
def compare_fstab_list(backup_path, ip_address):
    """
    Compares the pre-patch and post-patch fstab configuration information and saves the differences to a file.
    ...
    try:
        ...
        pre_patch_file = os.path.join(pre_patch_dir, 'fstab_Prepatch.txt')
        post_patch_file = os.path.join(post_patch_dir, 'fstab_Postpatch.txt')
        ...
        for key in pre_patch_info:
            if key in post_patch_info and pre_patch_info[key] != post_patch_info[key]:
                ...
                csv_res = csv_convert_function(ip_address,backup_path,"info_fstab",pre_patch_info[key],post_patch_info[key])
        ...
        if diff_info != {}:
            ...
            diff_info_dateTime_file = os.path.join(diff_backup_dir, 'diff_fstab_info.txt')
 
            with open(diff_info_dateTime_file, 'w') as f:
                json.dump(diff_info, f, indent=4)
 
            return {"changed": True, "msg": f"fstab info differ:\n{diff_info}"}
        else:
            return {"changed": False, "msg": "fstab info are identical"}
